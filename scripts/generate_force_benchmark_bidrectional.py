"""
Baseline video generation using Wan2.1-Fun-1.3B-InP (standard bidirectional I2V,
no distillation) on the Force Prompting benchmark.

This uses the stock WanI2V pipeline with bidirectional attention — no self-forcing
or DMD distillation checkpoints required. This is the correct baseline script to
run with just the Wan2.1-Fun-1.3B-InP checkpoint.

Contrast with generate_force_benchmark_causal.py, which runs the self-forcing
distilled model and requires additional ode_init and self_dmd checkpoints.

# from Self-Forcing root — original caption only
python scripts/generate_force_benchmark_bidrectional.py \
  --checkpoint_dir ./Wan2.1-Fun-1.3B-InP \
  --output_folder videos/wan_i2v_1_3b/original_prompt

# with force description appended to caption
python scripts/generate_force_benchmark_bidrectional.py \
  --checkpoint_dir ./Wan2.1-Fun-1.3B-InP \
  --output_folder videos/wan_i2v_1_3b \
  --append_force_text
...
python scripts/generate_force_benchmark_bidrectional.py \
  --checkpoint_dir ./Wan2.1-Fun-1.3B-InP \
  --output_folder videos/wan_i2v_1_3b/original_prompt \
  --benchmark_dir /users/jzou22/scratch/Self-Forcing/datasets/point-force/test/benchmark/apple
"""
import argparse
import glob
import os
...
from wan.configs import i2v_1_3B  # config object, not the module
from wan.image2video import WanI2V


BENCHMARK_DIR = "/users/jzou22/scratch/Self-Forcing/datasets/point-force/test/benchmark"
    ...
    for csv_path in csv_paths:
        ...
        for _, row in df.iterrows():
            ...
            if os.path.exists(image_path):
                ...
                click.echo(click.style(f"Warning: image not found: {image_path}", fg="red"))
    return samples


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint_dir", type=str, default="./Wan2.1-Fun-1.3B-InP",
                        help="Path to Wan2.1-Fun-1.3B-InP checkpoint directory")
    parser.add_argument("--benchmark_dir", type=str, default=BENCHMARK_DIR)
    parser.add_argument("--output_folder", type=str, default="videos/wan_i2v_1_3b")
    ...
    # load the stock bidirectional WanI2V — no self-forcing checkpoints needed.
    # t5_cpu keeps T5 on CPU to save VRAM; offload_model moves DiT on/off GPU per step.
    ...
    for i, (image_path, csv_stem, row) in enumerate(tqdm(samples, desc="samples")):
        caption = row["caption"]
        ...
        tqdm.write(click.style(f"[{i+1}/{len(samples)}] {csv_stem} | angle={angle:.1f} force={force:.2f}", fg="magenta"))
        tqdm.write(click.style(f"  prompt: {caption[:80]}{'...' if len(caption) > 80 else ''}", fg="white"))

        # benchmark images are 720x480; resize to 832x480 to match 480p resolution
        ...
        tqdm.write(click.style(f"  saved -> {fname}", fg="green"))
